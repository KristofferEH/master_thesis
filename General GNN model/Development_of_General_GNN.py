...
def extract_training_SMILES_temperature_and_target(excel_filename, path_to_code, number_of_temperatures, min_interval):
    """
    ...
    excel_data = pd.read_excel(f"{path_to_code}/Data/Min interval of {min_interval} ºC/{number_of_temperatures} temperatures/{excel_filename}")
...
def train_model_with_CV(total_dataset, num_gcn_layers, num_hidden_layers, num_dense_neurons, GCN_output_per_layer,
                        learning_rate, size_of_batch, dropout_rate, mean, std, activation_function, learning_rate_decay):
    """
    Perform CV to find the optimal number of epochs. 
    """
    ...
    for train_index, test_index in cross_validation.split(total_dataset):
        ...
        while initislize:
            ...
            opt = Adam(model.parameters(), lr=learning_rate)
            learning_rate_scheduler = ExponentialLR(opt, gamma=learning_rate_decay)
            loss_fn = torch.nn.MSELoss()
        ...
        if index == 1:
            ...
            if os.path.exists(f"Data/GCN Cross Validation/Validation_error.xlsx"):
                input("Are you sure you want to delete the existing file? Press enter to continue.")
                os.remove(f"Data/GCN Cross Validation/Validation_error.xlsx")

            # Have to make sure that both the folders exist. If it doesn't, create it.
            if not os.path.exists(f"Data/GCN Cross Validation"):
                os.mkdir(f"Data/GCN Cross Validation")

            # Save the dataframe to an Excel file.
            df_validation.to_excel(f"Data/GCN Cross Validation/Validation_error.xlsx", index=False)
            df_train.to_excel(f"Data/GCN Cross Validation/Train_error.xlsx", index=False)

        else:
            # Load the Excel file
            df_validation = pd.read_excel(f"Data/GCN Cross Validation/Validation_error.xlsx")
            df_train = pd.read_excel(f"Data/GCN Cross Validation/Train_error.xlsx")
...
def generate_hidden_dims(num_hidden_layers):
    """
    Defines the search space for the number of neurons in each dense layer
    """

    neurons_per_layer_options = list(range(100, 421, 40))  # Start at 100, stop at 420, step by 40

    # Randomly select the number of neuron for the specified number of layers. We use .choices() instead of .random(),
    # as we want to allow for repeated values.
    hidden_dims = random.choices(neurons_per_layer_options, k=num_hidden_layers)

    return hidden_dims


def generate_GCN_output_dims(num_gcn_layers):
    """
    Defines the search space for the number of neurons in each GCN layer
    """
    neurons_per_layer_options = list(range(100, 421, 40))  # Start at 60, stop at 420, step by 40

    # Randomly select the number of neuron for the specified number of layers. 
    hidden_dims = random.choices(neurons_per_layer_options, k=num_gcn_layers)
...
def train_with_CV(best_config):
    """
    Main execution function. This function will train a model using cross validation.
    """
    ...
    # Extracts data from the file that gave the best model.
    X_dev, y_dev, temp = extract_training_SMILES_temperature_and_target("Train.xlsx", path_to_code, number_of_temperatures, min_interval)
    ...
    learning_rate_decay = best_config["decay_rate"]
    ...
    with open(f"Data/GCN Cross Validation/Lowest_error.txt", "w") as file:
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"Data/GCN Cross Validation/Validation_error_plot.png", dpi=300)

    # Also want a plot that don't include the first 100 epochs.
    # Do not want to plot the first 100 elements
    plt.figure()
    average_validation_error = average_validation_error[10:]
    average_train_error = average_train_error[10:]
    # I want the x-axis to start at 100
    x_axis = list(range(10, len(average_validation_error) + 10))
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"Data/GCN Cross Validation/Validation_error_plot_excluding_first_100.png", dpi=300)
...
def search_hyperparameter_space():
    """
    Main execution function. This function is searches over the hyperparameter space using Ray Tune to find the best set of 
    hyperparameters. 
    """
    ...
    search_space = {
        "num_gcn_layers": tune.choice([3, 4]),
        "num_hidden_layers": tune.choice([4, 5, 6]),
        "GCN_output_per_layer": tune.sample_from(lambda config: generate_GCN_output_dims(
            config["num_gcn_layers"])),
        "hidden_neurons_per_layer": tune.sample_from(lambda config: generate_hidden_dims(
            config["num_hidden_layers"])),
        "learning_rate": tune.uniform(0.001, 0.01),
        "size_of_batch": tune.choice([32, 64, 128]),
        "number_of_epochs": 600,
        "number_of_temp": tune.choice([7, 8, 9, 10]),
        "min_interval": tune.choice([10, 20, 30, 40]),
        "patience": tune.choice([50]),
        "dropout_rate": tune.choice([0.2, 0.3]), 
        "activation_function": tune.choice(["relu"]),
        "decay_rate": tune.uniform(0.95, 0.99)
    }

    # Path to the folder where the code is located.
    path_to_code = os.path.dirname(os.path.realpath(__file__))

    # How many sets of the hyperparameter space we want to consider.
    num_configuration = 1000
    ...
    asha_scheduler = ASHAScheduler(
        max_t=search_space["number_of_epochs"], 
        grace_period=10,  # Minimum number of epochs before a trial can be stopped
        reduction_factor=3,  #  How many trials to keep after each round.
    )

    # Run the hyperparameter search
    results = tune.run(
        lambda config: train_model(config, path_to_code),
        config=search_space, 
    ...
    loss = results.best_result["loss"]

    print("Best hyperparameters found were: ", best_configuration)
    print("Minimum loss found was: ", loss)

    # Saves the best hyperparameters to a file.
    with open(f"Data/Best_hyperparameters.txt", "w") as file:
        file.write(str(best_configuration))
        file.write("\n")
        file.write(str(loss))

    # Starts CV with the best hyperparameters found to find the optimal number of epochs.
    train_with_CV(best_configuration)

#search_hyperparameter_space()
...
# using cross validation on the best hyperparameters found using Ray Tune.
...
def create_parity_plot(y_hat_values, target_values, temperatures, seed, train_or_test):
    ...
    plt.title(f"Parity Plot - {train_or_test}", fontsize=18)

    # Saves the figure in a folder
    plt.tight_layout()
    plt.savefig(f"Data/Final model/{seed}/Parity Plot - {train_or_test}", dpi=300)
    plt.close()
    ...
    plt.xlabel('Actual Values [log(mmHg)]', fontsize=18)
    plt.ylabel('Predicted Values [log(mmHg)]', fontsize=18)
    plt.title(f"Parity Plot - 40°C-70°C - {train_or_test}", fontsize=18)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"Data/Final model/{seed}/Parity Plot - 40°C-70°C - {train_or_test}", dpi=300)
    plt.close()
    ...
    plt.xlabel('Actual Values [log(mmHg)]', fontsize=18)
    plt.ylabel('Predicted Values [log(mmHg)]', fontsize=18)
    plt.title(f"Parity Plot - 70°C-100°C - {train_or_test}", fontsize=18)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"Data/Final model/{seed}/Parity Plot - 70°C-100°C - {train_or_test}", dpi=300)
    plt.close()
    ...
    plt.xlabel('Actual Values [log(mmHg)]', fontsize=18)
    plt.ylabel('Predicted Values [log(mmHg)]', fontsize=18)
    plt.title(f"Parity Plot - 100°C-135°C - {train_or_test}", fontsize=18)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f"Data/Final model/{seed}/Parity Plot - 100°C-135°C - {train_or_test}", dpi=300)
    plt.close()
...
def train_final_model(total_dataset, test_dataset, num_gcn_layers, num_hidden_layers, num_dense_neurons,
                      ...
                      mean, std, activation_function, seed, decay_rate, load_model=False, train_further=False):
    """
    ...
    # If we have not trained the model before, we want to train it.
    if not load_model:
        train_error_for_each_epoch = []
        model.train()
        # We run the code for "number_of_epochs" epochs.
        ...
        if not os.path.exists(f"Data/Final model"):
            os.mkdir(f"Data/Final model")

        # Create a folder with the seed number if it doesn't exist.
        if not os.path.exists(f"Data/Final model/{seed}"):
            os.mkdir(f"Data/Final model/{seed}")

        # Want to save the model
        torch.save(model.state_dict(), f"Data/Final model/{seed}/Best_model.pt")
        ...
        plt.legend()
        plt.tight_layout()
        plt.savefig(f"Data/Final model/{seed}/Training_error_plot.png", dpi=300)
        plt.close()

    else:
        # Want to load the saved model.
        model.load_state_dict(torch.load(f"Data/Final model/{seed}/Best_model.pt"))
    ...
    # Save the dataframe to an Excel file.
    df_train.to_excel(f"Data/Final model/{seed}/Predicted_values_train.xlsx", index=False)
    ...
    # Save the dataframe to an Excel file.
    df.to_excel(f"Data/Final model/{seed}/Predicted_values_test.xlsx", index=False)
    ...
    with open(f"Data/Final model/{seed}/Errors.txt", "w") as file:
    ...
    # Also want to save this to a text file.
    with open(f"Data/Final model/{seed}/Errors.txt", "a") as file:
        file.write(f"\nThe training error (RMSE) is {train_error} mmHg.")
        file.write("\n")
...
def final_model(best_config, seed):
    """
    Main execution function. This function will train the final model using the best hyperparameters found using Ray Tune.
    """
    ...
    X_dev, y_dev, temp_dev = extract_training_SMILES_temperature_and_target("Train.xlsx", path_to_code, number_of_temperatures, min_interval)
    X_test, y_test, temp_test = extract_training_SMILES_temperature_and_target("Test.xlsx", path_to_code, number_of_temperatures, min_interval)
...
# Create a list of 10 random seeds.
seeds = [12, 43, 23, 54, 65, 76, 87, 98, 109, 120]
...
for index, smiles in enumerate(df_pred["SMILES"].unique()):
    ...
    #if df_temp["Predicted log(P)"].iloc[-1] - df_temp["Predicted log(P)"].iloc[0] < 0.15:
